...
import TraceAnalysis.exception.*;
...
import TraceAnalysis.security.jwt.JwtConfigProperties;
import TraceAnalysis.security.jwt.JwtTokenUtil;
import TraceAnalysis.security.jwt.SampleManager;
...
public class UserServiceImpl implements UserService {
    ...
    public ResponseWrapper<?> findByUsername(String username) {
        User user = userRepository.findByUsername(username);
        if (user == null) {
            throw new UserNamedidntExistException(username);
        }//邀请对象是否存在
    ...
    public ResponseWrapper<?> findById(long id) {
        ...
        if (user == null) {
            throw new UserNamedidntExistException(Long.toString(id));
        }//邀请对象是否存在
        HashMap<String, HashMap<String, Object>> body = new HashMap<>();
        HashMap<String, Object> response = ResponseGenerator.generate(user,
                new String[]{"id", "username", "fullname", "email", "institution", "region"}, null);
    ...
    @Override
    public ResponseWrapper<?> findByFullnameAndEmail(String fullname, String email) {
        User user = userRepository.findByFullnameAndEmail(fullname, email);
        if (user == null) {
            throw new UserNamedidntExistException(fullname);
        }//邀请对象是否存在
        HashMap<String, HashMap<String, Object>> body = new HashMap<>();
        HashMap<String, Object> response = ResponseGenerator.generate(user,
                new String[]{"id", "username", "fullname", "email", "institution", "region"}, null);
