<template>
  ...
  >
    <v-col cols="10" class="mx-6 py-5">
      <v-list id="innerEssayForm" class="lighten-5 px-5 mx-5 align-center">
        <essayArticle
        ...
        <p class="display-0 font-weight-medium mt-2 pt-2 mb-1 pb-1">Topics</p>
        <topicOptions
          :initTopics="initArticleTopics"
          :availableTopics="meetingTopics"
          v-on:topicChange="filterTopics($event)"
          v-if="
            meetingTopicLoadCompleted &&
            (articleTopicloadCompleted || !updateArticle)
          "
        >
        </topicOptions>
        ...
        <p
          ...
          class="display-0 font-weight-medium mt-2 pt-2 mb-1 pb-1"
        >
          Upload your new essay pdf file(If you want to remain the old version
          pdf, ignore this step)
        </p>
        <v-file-input
...
export default {
  ...
  methods: {
    ...
    submitEssay: function (essayForm) {
      ...
      let requestUrl = "/api/article-service/user/articleSubmission";
      if (this.updateArticle) {
        formData.append("articleId", this.updateArticleId);
        requestUrl = "/api/article-service/user/updateArticle";
      }

      const OSS = require("ali-oss");
      // We hide information that could reveal our identity
      const client = new OSS({
        bucket: "ournote",
        region: "oss-xxxx-xxxxxxxxxx",
      ...
      async function put() {
        try {
          let result = await client.put(document.querySelector("input[type=file]").files[0].name, document.querySelector("input[type=file]").files[0]);
        } catch (e) {
          console.log(e);
        }
      }
      ...
      this.$axios
        ...
        .then((resp) => {
          if (
            resp.data.responseCode == 200 &&
            resp.data.responseMessage == "success"
          ) {
            this.tips_text = "Success upload your essay submission";
            this.$toast(this.tips_text, { color: "green" });
            var jumpUrl =
              "/submission/list?authorName=" +
              essayForm.username +
              "&meetingName=" +
              essayForm.meetingName;
            window.setTimeout(
              function () {
    ...
    loadUrlParams: function () {
      ...
      this.essayApplierForm.username = "C";
    ...
    loadMeetingTopics: function (meetingName) {
      this.$axios
        .get("/api/meeting-service/meeting/meetingInfo", {
    ...
    loadUploadedArticleDetail() {
      var requestUrl = "/api/article-service/user/articleDetail";
      this.$axios
        .get(requestUrl, { params: { articleId: this.updateArticleId } })
        .then((resp) => {
          // console.log(resp)
          if (
            resp.data.responseCode == 200 &&
            ...
            this.essayApplierForm.essayPDFUrl =
              "/api/user-service/utils/pdf?pdfUrl=" + ref.filePath;

            this.articleDeatilLoadCompleted = true;
            this.articleTopicloadCompleted = true;
          } else {
            this.tips_text =
              "error occurred due to " + resp.data.responseBody.reason;
            this.$toast(this.tips_text, { color: "red" });
          }
        })
        .catch((error) => {
